...
codeGilbert* GilbertMurCode(Symbol* chanceSymbols, short int numSymbols) {
...
float calAVGLength(codeGilbert* A, int n) {
    float result = 0;
    for (int i = 0; i < n; i++) {
        result += A[i].Li * A[i].Pi;
    }
...
void printGilbertCode(codeGilbert* C, int n) {
    wcout << L"\n??? ????????-????:" << endl;
    wcout << L"---------------------------------------------------------" << endl;
    wcout << L"| ?????? | ??????????? | ????? | ??????? ?????           |" << endl;
    wcout << L"---------------------------------------------------------" << endl;
...
float checkKraft(codeGilbert* C, int n) {
    float kraftSum = 0;
    for (int i = 0; i < n; i++) {
...
Tree* buildGilbertMurTree(codeGilbert* codes, int n) {
    Tree* root = new Tree{ L'\0', 0 };

    for (int i = 0; i < n; ++i) {
        Tree* p = root;
        for (int j = 0; j < codes[i].Li; ++j) {
            if (codes[i].codeword[j] == '0') {
...
int main() {
    ...
    RenderWindow window(VideoMode(1900, 800), "Gilbert Coding Tree");
    Font font;
    ...
    codeGilbert* C = GilbertMurCode(Symbols, nUnique);
    float AVGGilbert = calAVGLength(C, nUnique);

    float entropy = calEntropy(Symbols, nUnique);
    codeHuffman* C231 = HuffmanCode(Symbols, nUnique);
    ...
    wcout << L"|???????-???| " << fixed << setprecision(4) << AVGGilbert << L"                       |" << endl;
    wcout << L"--------------------------------------------" << endl;
    ...
    while (file1.get(ch)) {
        for (int i = 0; i < nUnique; i++) {
            if (C[i].ch == ch) {
                for (int j = 0; j < C[i].Li; j++) {
                    wcout << C[i].codeword[j];
                    encodedSequence += C[i].codeword[j];
                }
                wcout << L" ";
    ...
    Tree* gilbertTree = buildGilbertMurTree(C, nUnique);

    wstring decodedSequence = decode(gilbertTree, encodedSequence);
    ...
    while (window.isOpen()) {
        ...
        drawTree(window, gilbertTree, 950 + offsetX, 25 + offsetY, 470, font);
