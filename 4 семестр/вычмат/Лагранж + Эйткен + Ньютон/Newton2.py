...
def calculate_deltas(points_y):
    n = len(points_y)
    delta_y = [[0.0 for _ in range(n)] for _ in range(n)]

    for i in range(n):
        delta_y[i][0] = points_y[i]
...
def newton_second_formula(points_x, points_y, x_val):
    n = len(points_x)
    delta_y = calculate_deltas(points_y)
    h = points_x[1] - points_x[0]
    q = (x_val - points_x[-1]) / h

    result = points_y[-1]
    term = q

    for i in range(1, n):
        result += term * delta_y[n - 1 - i][i] / math.factorial(i)
        term *= (q + i)
...
def draw_graph(points_x, points_y, blue_x, blue_y, x_step=1.0):
    ...
    y_newton = [newton_second_formula(points_x, points_y, x) for x in x_vals]
    plt.plot(x_vals, y_newton, 'b-', label=f'Интерполяция Ньютона 2 (N={len(points_x)})', linewidth=2)
...
if __name__ == "__main__":
    try:
        ...
        blue_y = newton_second_formula(points_x, points_y, blue_x)
