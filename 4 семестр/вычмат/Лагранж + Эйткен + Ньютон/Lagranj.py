...
def draw_graph(points_x, points_y, blue_x, blue_y, x_step=10.0):
    ...
    if points_x[0] > 3:
        ...
        y_min, y_max = calculate_y_limits_old(y_lagrange, y_func, points_y)
        y_min = max(0, y_min)

    y_step = calculate_y_step(y_min, y_max)

    label = f'Интерполяция Лагранжа (N={len(points_x)})'
    plt.plot(x_vals, y_lagrange, 'b-', label=label, linewidth=2)
    ...
    plt.title(f'Интерполяция Лагранжа (N={len(points_x)}) для функции y = √x', fontsize=16)
