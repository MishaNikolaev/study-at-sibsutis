...
double EitkenInterpolation(double x_val) {
    double P[n][n];
    ...
    for (int j = 1; j < n; j++) {
        for (int i = 0; i < n - j; i++) {
            P[i][j] = ((x_val - x_arr[i + j]) * P[i][j - 1] - 
                      (x_val - x_arr[i]) * P[i + 1][j - 1]) / 
                      (x_arr[i] - x_arr[i + j]);
        }
    }
...
void draw_graph(double interpolated_y) {
    ...
    for (double xi = x_min; xi <= x_max; xi += step_size) {
        double yi = EitkenInterpolation(xi);
        if (!isnan(yi)) {
            int px = originX + (xi - x_min) * scaleX;
            int py = originY - (yi - y_min) * scaleY;
...
int main() {
    ...
    cin >> x0;
    if (x0 < 0) {
        cout << "x0 must be bigger than 0" << endl;
        return 1;
    }
    ...
    int gd = DETECT, gm;
    initwindow(1200, 800, "Eitken Interpolation Graph");
    ...
    calc_y();
    double y = EitkenInterpolation(x);
